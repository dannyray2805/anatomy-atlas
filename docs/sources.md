...
- [x] Map the exported systems' node names → Uberon ids, one layer at a time — skeleton (66 bones, 2026-09-13), muscle (132 muscles, 2026-09-14), organs/ducts/bronchi/brainstem (2026-09-12/13), and the vessel tree (**229 of the layer's 416 named vessels, 2026-09-14**, covering 383 loader mesh names). Every id is accepted only where the asset's own name is exactly an Uberon label **or one of its exact synonyms**, both read from the OLS4 term endpoint; every `mesh_names` entry comes from a real loader dump rather than from the file's node names. Still unmapped, with the reason stated rather than left silent: peripheral nerves (`nervous-v1`), joints (`joints-v1`), the hand-and-foot bones, the named muscle heads/bellies Uberon does not model, and 166 vessel names that are segmental vessels (of the lung) or small named branches. A part is never mapped to its whole: the M1/M3 segments of the middle cerebral artery, the abdominal/thoracic parts of the inferior vena cava and the divisions of the internal iliac artery stay unmapped.
